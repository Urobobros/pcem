...
- [ ] Ověřit, že ROM segmenty (0xC0000–0xFFFFF a případně FFFF0000h) jsou
...
- [ ] Dopsat logiku do `i430vx_write()` a `pam_update()`, aby změny PAMx
  registrů okamžitě přemapovaly GPA na RAM nebo ROM i při použití WHPX.
- [ ] Zajistit, aby při `resetx86()` a `init_real_mode_registers()` byl segment
...
- [ ] Implementovat přemapování (`WHvMapGpaRange`) při každé změně PAM
