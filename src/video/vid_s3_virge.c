...
static void s3_virge_out(uint16_t addr, uint8_t val, void *p) {
        ...
        case 0x3d5:
                ...
                case 0x4f:
                        ...
                        svga->hwcursor.addr =
                                ((((svga->crtc[0x4c] << 8) | svga->crtc[0x4d]) & 0xfff) * 1024) + (svga->hwcursor.yoff * 16);
                        break;
...
static void s3_virge_recalctimings(svga_t *svga) {
        ...
        if ((svga->crtc[0x67] & 0xc) != 0xc) /*VGA mode*/
        {
                ...
                svga->vram_display_mask =
                        (!(svga->crtc[0x31] & 0x08) && (svga->crtc[0x32] & 0x40)) ? 0x3ffff : ((virge->memory_size << 20) - 1);
        ...
        } else /*Streams mode*/
        {
                ...
                // svga->vtotal, svga->overlay.x, svga->overlay.y, svga->overlay.ysize);
...
#define CLIP(x, y)                                                                                                               \
        do {                                                                                                                     \
                if ((virge->s3d.cmd_set & CMD_SET_HC) &&                                                                         \
                    (x < virge->s3d.clip_l || x > virge->s3d.clip_r || y < virge->s3d.clip_t || y > virge->s3d.clip_b))          \
                        update = 0;                                                                                              \
        } while (0)
...
#define CLIP_3D(x, y)                                                                                                            \
        do {                                                                                                                     \
                if ((s3d_tri->cmd_set & CMD_SET_HC) &&                                                                           \
                    (x < s3d_tri->clip_l || x > s3d_tri->clip_r || y < s3d_tri->clip_t || y > s3d_tri->clip_b))                  \
                        update = 0;                                                                                              \
        } while (0)
        ...
        do {                                                                                                                     \
                ...
                case 1: /*16 bpp*/                                                                                               \
                        *(uint16_t *)&vram[addr & svga->vram_mask] = val;                                                        \
                        ...
                        break;                                                                                                   \
                case 2: /*24 bpp*/                                                                                               \
                        *(uint32_t *)&vram[addr & svga->vram_mask] =                                                             \
                                (val & 0xffffff) | (vram[(addr + 3) & svga->vram_mask] << 24);                                   \
                        virge->svga.changedvram[(addr & svga->vram_mask) >> 12] = changeframecount;                              \
                        break;                                                                                                   \
                }                                                                                                                \
        } while (0)
...
static void s3_virge_bitblt(virge_t *virge, int count, uint32_t cpu_dat) {
        ...
        case CMD_SET_COMMAND_BITBLT:
                ...
                while (count) {
                        uint32_t src_addr =
                                virge->s3d.src_base + (virge->s3d.src_x * x_mul) + (virge->s3d.src_y * virge->s3d.src_str);
                        uint32_t dest_addr =
                                virge->s3d.dest_base + (virge->s3d.dest_x * x_mul) + (virge->s3d.dest_y * virge->s3d.dest_str);
        ...
        case CMD_SET_COMMAND_LINE:
                ...
                while (virge->s3d.h) {
                        ...
                        if (virge->s3d.h == virge->s3d.lycnt &&
                            ((virge->s3d.line_dir && x > virge->s3d.lxend0) || (!virge->s3d.line_dir && x < virge->s3d.lxend0)))
                                x = virge->s3d.lxend0;
                        ...
                        do {
                                uint32_t dest_addr =
                                        virge->s3d.dest_base + (x * x_mul) + (virge->s3d.dest_y * virge->s3d.dest_str);
                                uint32_t source = 0, dest, pattern;
                                uint32_t out = 0;
                                int update = 1;
                                ...
                                if ((virge->s3d.h == 1 || !first_pixel) && ((virge->s3d.line_dir && x > virge->s3d.lxend1) ||
        ...
        case CMD_SET_COMMAND_POLY:
                ...
                virge->s3d.rop = (virge->s3d.cmd_set >> 17) & 0xff;
                // pclog("Start poly - l=%08x r=%08x h=%i rop=%02x\n", virge->s3d.dest_l, virge->s3d.dest_r, virge->s3d.h,
                // virge->s3d.rop);
                while (virge->s3d.h) {
...
static void tex_sample_normal_filter(s3d_state_t *state) {
        ...
        state->dest_rgba.r =
                (tex_samples[0].r * d[0] + tex_samples[1].r * d[1] + tex_samples[2].r * d[2] + tex_samples[3].r * d[3]) >> 16;
        state->dest_rgba.g =
                (tex_samples[0].g * d[0] + tex_samples[1].g * d[1] + tex_samples[2].g * d[2] + tex_samples[3].g * d[3]) >> 16;
        state->dest_rgba.b =
                (tex_samples[0].b * d[0] + tex_samples[1].b * d[1] + tex_samples[2].b * d[2] + tex_samples[3].b * d[3]) >> 16;
        state->dest_rgba.a =
                (tex_samples[0].a * d[0] + tex_samples[1].a * d[1] + tex_samples[2].a * d[2] + tex_samples[3].a * d[3]) >> 16;
}
...
static void tri(virge_t *virge, s3d_t *s3d_tri, s3d_state_t *state, int yc, int32_t dx1, int32_t dx2) {
        ...
        for (; y_count > 0; y_count--) {
                ...
                if (x != xe && ((x_dir > 0 && x < xe) || (x_dir < 0 && x > xe))) {
                        ...
                        //                        pclog("Draw Y=%i X=%i to XE=%i  %i   %08x %08x %08x %08x  %08x %08x %08x %08x %i
                        //                        %08x\n", state->y, x, xe, dx, state->x1, state->x2, dx1, virge->s3d.TdWdX,
                        //                        state->u, state->v, virge->s3d.TdUdX, virge->s3d.TdUdY, dx, (virge->s3d.TdUdX *
                        //                        dx) >> 4);
...
static void s3_virge_triangle(virge_t *virge, s3d_t *s3d_tri) {
        ...
        //        pclog("Triangle %i %i,%i to %i,%i  %08x\n", y, x1 >> 20, y, s3d_tri->txend01 >> 20, y - (s3d_tri->ty01 +
        //        s3d_tri->ty12), state.cmd_set);
...
static void s3_virge_hwcursor_draw(svga_t *svga, int displine) {
        ...
        //        virge->hwcursor_col[0],virge->hwcursor_col[1]);
        switch (svga->bpp) {
        case 15:
                ...
                if (svga->crtc[0x55] & 0x10) {
                        ...
                        for (xx = 0; xx < 16; xx++) {
                                if (offset >= svga->hwcursor_latch.x) {
                                        ...
                                        //                                displine, x+xx, svga->hwcursor_on, dat[0], dat[1]);
                                }
...
static void *s3_virge_init() {
        ...
        mem_mapping_add(&virge->mmio_mapping, 0, 0, s3_virge_mmio_read, s3_virge_mmio_read_w, s3_virge_mmio_read_l,
                        s3_virge_mmio_write, s3_virge_mmio_write_w, s3_virge_mmio_write_l, NULL, 0, virge);
        mem_mapping_add(&virge->new_mmio_mapping, 0, 0, s3_virge_mmio_read, s3_virge_mmio_read_w, s3_virge_mmio_read_l,
                        s3_virge_mmio_write, s3_virge_mmio_write_w, s3_virge_mmio_write_l, NULL, 0, virge);
        mem_mapping_add(&virge->linear_mapping, 0, 0, svga_read_linear, svga_readw_linear, svga_readl_linear, svga_write_linear,
                        svga_writew_linear, svga_writel_linear, NULL, 0, &virge->svga);
...
static void s3_virge_add_status_info(char *s, int max_len, void *p) {
        ...
        sprintf(temps, "%f Mpixels/sec\n%f ktris/sec\n%f%% CPU\n%f%% CPU (real)\n%d writes %i reads\n\n",
                ...
                ((double)virge_time * 100.0) / timer_freq, ((double)virge_time * 100.0) / status_diff, reg_writes, reg_reads);
...
device_t s3_virge_device = {"Diamond Stealth 3D 2000 (S3 ViRGE)",
                            0,
                            s3_virge_init,
                            s3_virge_close,
                            s3_virge_available,
...
device_t s3_virge_375_device = {"S3 ViRGE/DX",          0,
                                s3_virge_375_init,      s3_virge_close,
