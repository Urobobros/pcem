...
void x86_doabrt(int x86_abrt) {
        ...
        //        pclog("GPF! - error %04X  %04X(%08X):%08X %02X %02X  %i  %04X %i
                ...
                if (s == &cpu_state.seg_ss) {
                        ...
                        case 0x1B: /*Readable non-conforming code*/
                                ...
                                if ((seg & 3) > dpl || (CPL) > dpl) {
                                        ...
                                        x86gpf(NULL, seg & ~3);
                                        //                                        x86abort("Data segment load - level too
                                        //                                        low!\n",seg&0xFFFC);
                                        return 1;
                                }
                                break;
...
void loadcscall(uint16_t seg, uint32_t old_pc) {
        ...
        if (msw & 1 && !(cpu_state.eflags & VM_FLAG)) {
                ...
                if (csout)
                        pclog("Code seg call - %04X - %04X %04X %04X\n", seg, segdat[0], segdat[1], segdat[2]);
                if (segdat[2] & 0x1000) {
                        if (!(segdat[2] & 0x400)) /*Not conforming*/
                        {
                                if ((seg & 3) > CPL) {
                                        if (csout)
                                                pclog("Not conforming, RPL > CPL\n");
                                        x86gpf(NULL, seg & ~3);
                                        return;
                                }
                                if (CPL != DPL) {
                                        if (csout)
                                                pclog("Not conforming, CPL != DPL (%i %i)\n", CPL, DPL);
                                        x86gpf(NULL, seg & ~3);
                                        return;
                                }
                        }
                        ...
                        case 0xC00: /*386 Call gate*/
                                ...
                                if (!(segdat[2] & 0x8000)) {
                                        if (output)
                                                pclog("Call gate not present %04X\n", seg);
                                ...
                                if (output)
                                        pclog("Code seg2 call - %04X - %04X %04X %04X\n", seg2, segdat[0], segdat[1], segdat[2]);
                                ...
                                if (!(segdat[2] & 0x8000)) {
                                        if (output)
                                                pclog("Call gate CS not present %04X\n", seg2);
                                ...
                                case 0x1B00: /*Non-conforming code*/
                                        if (DPL < CPL) {
                                                ...
                                                if (((newss & 3) != DPL) || (DPL2 != DPL)) {
                                                        pclog("Call gate loading SS with wrong permissions  %04X %04X  %i %i   "
                                                              "%04X %04X\n",
...
void pmoderetf(int is32, uint16_t off) {
        ...
        if (CPL == (seg & 3)) {
                ...
                if (output)
                        pclog("Read new stack : %04X:%04X (%08X)\n", newss, newsp, ldt.base);
                if (!(newss & ~3)) {
...
void pmodeint(int num, int soft) {
        ...
        if (output)
                pclog("Addr %08X seg %04X %04X %04X %04X\n", addr, segdat[0], segdat[1], segdat[2], segdat[3]);
        if (!(segdat[2] & 0x1F00)) {
                //                pclog("No seg\n");
                if (cpu_state.eflags &
                    VM_FLAG) /*This fires on all V86 interrupts in EMM386. Mark as expected to prevent code churn*/
...
void pmodeiret(int is32) {
        ...
        if (is32) {
                ...
                if (is386 && ((tempflags >> 16) & VM_FLAG)) {
                        ...
                        //                        pclog("V86 IRET to %04X:%04X  %04X:%04X  %04X %04X %04X %04X
...
void taskswitch286(uint16_t seg, uint16_t *segdat, int is32) {
        ...
        if (is32) {
                ...
                if (optype == JMP || optype == CALL || optype == OPTYPE_INT) {
                        ...
                        tempw |= 0x200;
                        if (tr.seg & 4)
                                writememw(ldt.base, (seg & ~7) + 4, tempw);
                        else
                ...
                if (optype == JMP || optype == IRET) {
                        ...
                        tempw &= ~0x200;
                        if (tr.seg & 4)
                                writememw(ldt.base, (tr.seg & ~7) + 4, tempw);
                        else
...
void x86_smi_trigger(void) { cpu_state.smi_pending = 1; }
...
void cyrix_write_seg_descriptor(uint32_t addr, x86seg *seg) {
        ...
        writememl(0, addr + 4,
                  ((seg->base >> 16) & 0xff) | (seg->access << 8) | (seg->limit_raw & 0xf0000) | (seg->access2 << 16) |
                          (seg->base & 0xff000000));
}
