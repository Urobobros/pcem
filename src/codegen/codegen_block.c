...
#endif
        //        pclog("block_dirty_list_add: block=%i size=%i %i head=%i tail=%i\n", get_block_nr(block), dirty_list_size,
        //        dirty_list_check_size(), block_dirty_list_head, block_dirty_list_tail);
...
void dump_block() {
        /*        codeblock_t *block = pages[0x119000 >> 12].block;
           ...
           (void *)block->prev, (void *)block->next); if (!block->pc) fatal("Dead PC=0\n");
...
static void add_to_block_list(codeblock_t *block) {
        ...
        if (block->next) {
                ...
                if (codeblock[block->next].pc == BLOCK_PC_INVALID)
                        fatal("block->next->pc=BLOCK_PC_INVALID %p %p %x %x\n", (void *)&codeblock[block->next],
                              (void *)codeblock, block_current, block_pos);
#endif
        }
...
static void remove_from_block_list(codeblock_t *block, uint32_t pc) {
        ...
        if (block->prev_2) {
                ...
                //                pclog(" pages.block_2=%p 3 %p %p\n", (void *)block->next_2, (void *)block, (void
                //                *)pages[block->phys_2 >> 12].block_2);
...
void codegen_check_flush(page_t *page, uint64_t mask, uint32_t phys_addr) {
        ...
        while (block_nr) {
                ...
                if (*block->dirty_mask & block->page_mask) {
                        //                        pclog("Delete block from codegen_check_flush %08x %08x  %016llx %016llx %016llx
...
ir_data_t *codegen_get_ir_data() { return ir_data; }
...
void codegen_block_start_recompile(codeblock_t *block) {
        ...
        cpu_state.seg_ds.checked = cpu_state.seg_es.checked = cpu_state.seg_fs.checked = cpu_state.seg_gs.checked =
                (cr0 & 1) ? 0 : 1;
