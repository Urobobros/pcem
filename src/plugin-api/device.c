...
device_t *model_getdevice(int model) { return models[model]->device; }

void device_init() { memset(devices, 0, sizeof(devices)); }

void device_add(device_t *d) { pcem_add_device(d); }
