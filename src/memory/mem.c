...
static inline uint32_t mmu_readl(uint32_t addr) { return *(uint32_t *)&_mem_exec[addr >> 14][addr & 0x3fff]; }

static inline void mmu_writel(uint32_t addr, uint32_t val) { *(uint32_t *)&_mem_exec[addr >> 14][addr & 0x3fff] = val; }
...
uint32_t mmutranslatereal(uint32_t addr, int rw) {
        ...
        //        /*if (output) */pclog("Translate %08X %08X %08X  %08X:%08X
...
void addwritelookup(uint32_t virt, uint32_t phys) {
        ...
        if (pages[phys >> 12].block || (phys & ~0xfff) == recomp_page)
                page_lookup[virt >> 12] =
                        &pages[phys >> 12]; //(uintptr_t)&ram[(uintptr_t)(phys & ~0xFFF) - (uintptr_t)(virt & ~0xfff)];
        else
                writelookup2[virt >> 12] = (uintptr_t)&ram[(uintptr_t)(phys & ~0xFFF) - (uintptr_t)(virt & ~0xfff)];
        //        pclog("addwritelookup %08x %08x %p %p %016llx %p\n", virt, phys, (void *)page_lookup[virt >> 12], (void
        //        *)writelookup2[virt >> 12], pages[phys >> 12].dirty_mask, (void *)&pages[phys >> 12]);
...
static inline int page_index(page_t *p) { return ((uintptr_t)p - (uintptr_t)pages) / sizeof(page_t); }
...
void mem_write_ramb_page(uint32_t addr, uint8_t val, page_t *p) {
        if (val != p->mem[addr & 0xfff] || codegen_in_recompile) {
                ...
                if ((p->byte_code_present_mask[byte_offset] & byte_mask) && !page_in_evict_list(p)) {
                        //                        pclog("   ramb add %08x %016llx %016llx\n", addr,
                        //                        p->byte_code_present_mask[byte_offset], byte_mask);
                        page_add_to_evict_list(p);
                }
        }
}
void mem_write_ramw_page(uint32_t addr, uint16_t val, page_t *p) {
        if (val != *(uint16_t *)&p->mem[addr & 0xfff] || codegen_in_recompile) {
                ...
                //        cs+cpu_state.pc, p->dirty_mask[index], p->code_present_mask[index], mask, p->evict_prev, p->evict_next,
                //        p);
                *(uint16_t *)&p->mem[addr & 0xfff] = val;
                ...
                if ((addr & PAGE_BYTE_MASK_MASK) == PAGE_BYTE_MASK_MASK) {
                        ...
                        if ((p->byte_code_present_mask[byte_offset + 1] & 1) && !page_in_evict_list(p)) {
                                //                                pclog("ramw add %08x %016llx %016llx\n", addr,
                                //                                p->code_present_mask[index], mask);
                                page_add_to_evict_list(p);
                        }
                } else
...
void mem_write_raml_page(uint32_t addr, uint32_t val, page_t *p) {
        if (val != *(uint32_t *)&p->mem[addr & 0xfff] || codegen_in_recompile) {
                ...
                //        p->dirty_mask[index], p->code_present_mask[index], mask);
                *(uint32_t *)&p->mem[addr & 0xfff] = val;
                p->dirty_mask |= mask;
                p->byte_dirty_mask[byte_offset] |= byte_mask;
                if (!page_in_evict_list(p) &&
                    ((p->code_present_mask & mask) || (p->byte_code_present_mask[byte_offset] & byte_mask))) {
...
uint8_t mem_read_romext(uint32_t addr, void *priv) { return romext[addr & 0x7fff]; }
...
static void mem_mapping_recalc(uint64_t base, uint64_t size) {
        ...
        while (mapping != NULL) {
                /*In range?*/
                if (mapping->enable && (uint64_t)mapping->base < ((uint64_t)base + (uint64_t)size) &&
                    ((uint64_t)mapping->base + (uint64_t)mapping->size) > (uint64_t)base) {
                        uint64_t start = (mapping->base < base) ? mapping->base : base;
                        uint64_t end = (((uint64_t)mapping->base + (uint64_t)mapping->size) < (base + size))
                                               ? ((uint64_t)mapping->base + (uint64_t)mapping->size)
                                               : (base + size);
...
void mem_mapping_set_p(mem_mapping_t *mapping, void *p) { mapping->p = p; }
...
void mem_remap_top_256k() { mem_remap_top(256); }
void mem_remap_top_384k() { mem_remap_top(384); }
...
void mem_alloc() {
        ...
        if (romset == ROM_XI8088) {
                ...
                mem_set_mem_state(0x0c0000, 0x08000,
                                  model_get_config_int("umb_c0000h_c7fff") ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                                           : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
                mem_set_mem_state(0x0c8000, 0x08000,
                                  model_get_config_int("umb_c8000h_cffff") ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                                           : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
                mem_set_mem_state(0x0d0000, 0x08000,
                                  model_get_config_int("umb_d0000h_d7fff") ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                                           : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
                mem_set_mem_state(0x0d8000, 0x08000,
                                  model_get_config_int("umb_d8000h_dffff") ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                                           : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
                mem_set_mem_state(0x0e0000, 0x08000,
                                  model_get_config_int("umb_e0000h_e7fff") ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                                           : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
                mem_set_mem_state(0x0e8000, 0x08000,
                                  model_get_config_int("umb_e8000h_effff") ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                                           : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
        ...
        mem_mapping_add(&ram_low_mapping, 0x00000, (mem_size > 640) ? 0xa0000 : mem_size * 1024, mem_read_ram, mem_read_ramw,
                        mem_read_raml, mem_write_ram, mem_write_ramw, mem_write_raml, ram, MEM_MAPPING_INTERNAL, NULL);
        if (mem_size > 1024) {
                if (cpu_16bitbus && mem_size > 16256) {
                        mem_set_mem_state(0x100000, (16256 - 1024) * 1024, MEM_READ_INTERNAL | MEM_WRITE_INTERNAL);
                        mem_mapping_add(&ram_high_mapping, 0x100000, ((16256 - 1024) * 1024), mem_read_ram, mem_read_ramw,
                                        mem_read_raml, mem_write_ram, mem_write_ramw, mem_write_raml, ram + 0x100000,
                                        MEM_MAPPING_INTERNAL, NULL);
                } else {
                        mem_set_mem_state(0x100000, (mem_size - 1024) * 1024, MEM_READ_INTERNAL | MEM_WRITE_INTERNAL);
                        mem_mapping_add(&ram_high_mapping, 0x100000, ((mem_size - 1024) * 1024), mem_read_ram, mem_read_ramw,
                                        mem_read_raml, mem_write_ram, mem_write_ramw, mem_write_raml, ram + 0x100000,
                                        MEM_MAPPING_INTERNAL, NULL);
                }
        }
