...
const float sb_bass_treble_4bits[] = {0.199526231, 0.25, 0.316227766, 0.398107170, 0.5, 0.63095734, 0.794328234, 1, 0, 0.25892541,
                                      0.584893192, 1,    1.511886431, 2.16227766,  3,   4.011872336};

/* Attenuation tables for the mixer. Max volume = 32767 in order to give 6dB of
 * headroom and avoid integer overflow */
const int32_t sb_att_2dbstep_5bits[] = {25,   32,   41,   51,   65,    82,    103,   130,   164,   206,  260,
...
static void sb_get_buffer_sb16(int32_t *buffer, int len, void *p) {
        ...
        for (c = 0; c < len * 2; c += 2) {
                ...
                in_l = (mixer->input_selector_left & INPUT_MIDI_L)       ? out_l
                       : 0 + (mixer->input_selector_left & INPUT_MIDI_R) ? out_r
                                                                         : 0;
                in_r = (mixer->input_selector_right & INPUT_MIDI_L)       ? out_l
                       : 0 + (mixer->input_selector_right & INPUT_MIDI_R) ? out_r
                                                                          : 0;
        ...
        for (c = 0; c < len * 2; c += 2) {
                ...
                if (sb->dsp.sb_enable_i) {
                        ...
                        if (c_record > 0xFFFF && !buf_written) {
                                if (!soundfsb)
                                        soundfsb = fopen("sound_sb.pcm", "wb");
                        ...
                        if (c_record != last_crecord) {
                                if (!soundfsbin)
                                        soundfsbin = fopen("sound_sb_in.pcm", "wb");
...
static device_config_t sb_config[] = {
        {.name = "addr",
         ...
         .selection = {{.description = "0x210", .value = 0x210},
        ...
        {.name = "irq",
         .description = "IRQ",
         .type = CONFIG_SELECTION,
         .selection = {{.description = "IRQ 2", .value = 2},
        ...
        {.name = "dma",
         .description = "DMA",
         .type = CONFIG_SELECTION,
         .selection = {{.description = "DMA 1", .value = 1}, {.description = "DMA 3", .value = 3}, {.description = ""}},
         .default_int = 1},
        {.type = -1}};
...
static device_config_t sb2_config[] = {
        {.name = "addr",
         .description = "Address",
         .type = CONFIG_SELECTION,
         .selection = {{.description = "0x220", .value = 0x220}, {.description = "0x240", .value = 0x240}, {.description = ""}},
         .default_int = 0x220},
        {.name = "mixaddr",
         .description = "Mixer Address",
         .type = CONFIG_SELECTION,
         .selection = {{.description = "No mixer", .value = 0},
                       {.description = "0x250", .value = 0x250},
                       {.description = "0x260", .value = 0x260},
                       {.description = ""}},
         .default_int = 0},
...
static device_config_t sb_pro_mcv_config[] = {{.name = "opl_emu",
                                               .description = "OPL emulator",
                                               .type = CONFIG_SELECTION,
                                               .selection =
                                                       {
...
static device_config_t sb_16_config[] = {
        {.name = "addr",
         .description = "Address",
         .type = CONFIG_SELECTION,
         .selection = {{.description = "0x220", .value = 0x220},
                       {.description = "0x240", .value = 0x240},
                       {.description = "0x260", .value = 0x260},
                       {.description = "0x280", .value = 0x280},
                       {.description = ""}},
         .default_int = 0x220},
        {.name = "midi", .description = "MIDI out device", .type = CONFIG_MIDI, .default_int = 0},
        {.name = "opl_emu",
         .description = "OPL emulator",
         .type = CONFIG_SELECTION,
         .selection =
...
static device_config_t sb_awe32_config[] = {
        ...
        {.name = "emu_addr",
         ...
         .type = CONFIG_SELECTION,
         .selection = {{.description = "0x620", .value = 0x620},
        ...
        {.name = "onboard_ram",
         ...
         .selection = {{.description = "None", .value = 0},
                       ...
                       {.description = "28 MB", .value = 28 * 1024},
                       {.description = ""}},
         .default_int = 512},
        {.name = "opl_emu",
         .description = "OPL emulator",
...
device_t sb_1_device = {"Sound Blaster v1.0", 0,    sb_1_init,          sb_close, NULL,
                        sb_speed_changed,     NULL, sb_add_status_info, sb_config};
device_t sb_15_device = {"Sound Blaster v1.5", 0,    sb_15_init,         sb_close, NULL,
                         sb_speed_changed,     NULL, sb_add_status_info, sb_config};
device_t sb_mcv_device = {"Sound Blaster MCV", DEVICE_MCA, sb_mcv_init,        sb_close,     NULL,
                          sb_speed_changed,    NULL,       sb_add_status_info, sb_mcv_config};
device_t sb_2_device = {"Sound Blaster v2.0", 0,    sb_2_init,          sb_close,  NULL,
                        sb_speed_changed,     NULL, sb_add_status_info, sb2_config};
device_t sb_pro_v1_device = {"Sound Blaster Pro v1", 0,    sb_pro_v1_init,     sb_close,        NULL,
                             sb_speed_changed,       NULL, sb_add_status_info, sb_pro_v1_config};
device_t sb_pro_v2_device = {"Sound Blaster Pro v2", 0,    sb_pro_v2_init,     sb_close,        NULL,
                             sb_speed_changed,       NULL, sb_add_status_info, sb_pro_v2_config};
device_t sb_pro_mcv_device = {"Sound Blaster Pro MCV", DEVICE_MCA, sb_pro_mcv_init,    sb_close,         NULL,
                              sb_speed_changed,        NULL,       sb_add_status_info, sb_pro_mcv_config};
